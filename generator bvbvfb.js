...
let btnvfb = document.getElementById("button-vfb-goals");
let outputvfb = document.getElementById("result-vfb-goals");
...
btnvfb.addEventListener('click', () => {
	if (numbersArray.length == 0) {
		outputvfb.innerText = "No more random numbers";
		return;
	}
	let Solution = getRandomNumber(0, numbersArray.length-1);
	let randomNumber= numbersArray[Solution];
	numbersArray.slice(Solution, 1);
	outputvfb.innerText = randomNumber;
});// JavaScript Document