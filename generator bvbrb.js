...
let btnrb = document.getElementById("button-rb-goals");
let outputrb = document.getElementById("result-rb-goals");
...
btnrb.addEventListener('click', () => {
	if (numbersArray.length == 0) {
		outputrb.innerText = "No more random numbers";
		return;
	}
	let Solution = getRandomNumber(0, numbersArray.length-1);
	let randomNumber= numbersArray[Solution];
	numbersArray.slice(Solution, 1);
	outputrb.innerText = randomNumber;
});// JavaScript Document