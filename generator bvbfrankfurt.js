...
let btnfrankfurt = document.getElementById("button-frankfurt-goals");
let outputfrankfurt = document.getElementById("result-frankfurt-goals");
...
btnfrankfurt.addEventListener('click', () => {
	if (numbersArray.length == 0) {
		outputfrankfurt.innerText = "No more random numbers";
		return;
	}
	let Solution = getRandomNumber(0, numbersArray.length-1);
	let randomNumber= numbersArray[Solution];
	numbersArray.slice(Solution, 1);
	outputfrankfurt.innerText = randomNumber;
});// JavaScript Document