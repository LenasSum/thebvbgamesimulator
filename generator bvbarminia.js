...
let btnarminia = document.getElementById("button-arminia-goals");
let outputarminia = document.getElementById("result-arminia-goals");
...
btnarminia.addEventListener('click', () => {
	if (numbersArray.length == 0) {
		outputarminia.innerText = "No more random numbers";
		return;
	}
	let Solution = getRandomNumber(0, numbersArray.length-1);
	let randomNumber= numbersArray[Solution];
	numbersArray.slice(Solution, 1);
	outputarminia.innerText = randomNumber;
});// JavaScript Document