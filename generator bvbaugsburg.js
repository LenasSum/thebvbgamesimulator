...
let btnaugsburg = document.getElementById("button-augsburg-goals");
let outputaugsburg = document.getElementById("result-augsburg-goals");
...
btnaugsburg.addEventListener('click', () => {
	if (numbersArray.length == 0) {
		outputaugsburg.innerText = "No more random numbers";
		return;
	}
	let Solution = getRandomNumber(0, numbersArray.length-1);
	let randomNumber= numbersArray[Solution];
	numbersArray.slice(Solution, 1);
	outputaugsburg.innerText = randomNumber;
});// JavaScript Document