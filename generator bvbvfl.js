...
let btnvfl = document.getElementById("button-vfl-goals");
let outputvfl = document.getElementById("result-vfl-goals");
...
btnvfl.addEventListener('click', () => {
	if (numbersArray.length == 0) {
		outputvfl.innerText = "No more random numbers";
		return;
	}
	let Solution = getRandomNumber(0, numbersArray.length-1);
	let randomNumber= numbersArray[Solution];
	numbersArray.slice(Solution, 1);
	outputvfl.innerText = randomNumber;
});// JavaScript Document